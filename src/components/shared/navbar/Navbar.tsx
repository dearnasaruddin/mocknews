    ...
    pathname.includes('/auth') ?
      ...
      <header className='bg-gray-50 dark:bg-gray-900 shadow-md py-3 lg:py-4 fixed w-full top-0 z-50'>
        <nav className='flex items-center justify-between container'>
          ...
          {/* =========== Navigation Menu =========== */}
          <div className={` ${isMenuOpen ? ' max-md:right-0' : 'hidden max-md:-right-full'} lg:flex lg:items-center lg:justify-between lg:gap-6 duration-200  max-md:absolute max-md:top-0 max-md:flex max-md:flex-col max-md:justify-between max-md:w-[60vw] max-md:bg-gray-100 max-md:dark:bg-gray-800 max-md:shadow-2xl max-md:h-screen max-md:p-8 max-md:z-50`}>
            <IoMdCloseCircleOutline onClick={toggleMenu} className='absolute top-5 right-5 text-2xl lg:hidden' />
            <div >
              <NavigationMenu className='lg:flex' viewport={false}>
                <NavigationMenuList>
            ...
            <div className='flex items-center max-md:justify-center'>
              <ModeToggle />
              <Link href="/auth/login" className='ml-4 py-2 px-6 rounded-md text-sm font-medium bg-[#0f172b] dark:bg-[#e2e8f0] hover:bg-gray-700 dark:hover:bg-gray-300 duration-200 text-[#f9fafb] dark:text-[#3e4657]'>Login</Link>
            </div>
          </div>
